...
exports.sendWeeklyReport = async (req, res) => {
  try {
    ...
    if (!users.length) {
      if (res) return res.status(404).json({ message: "No users found" });
      console.log("No users found for weekly report");
      return;
    }
    ...
    if (res) {
      return res.status(200).json({ message: "Weekly reports sent successfully!" });
    } else {
      console.log("✅ Weekly reports sent successfully (cron mode)");
    }
  } catch (error) {
