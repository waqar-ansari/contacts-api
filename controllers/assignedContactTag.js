...
const assignOrUnassignTag = async (req, res) => {
  ...
  let { tagId, add = [], remove = [], tag: newTagName, emoji: newEmoji } = req.body;
  ...
  if (!tagId) {
    if (!newTagName) {
      return res.status(400).json({
        status: "error",
        message: "tagId or tag name is required",
      });
    }
    ...
    if (existingTag) {
      // If tag exists, set tagId to reuse it
      const found = existingTag.tags.find(t => t.tag === newTagName);
      tagId = found.tag_id;
    } else {
      // Create new tag
