...
function normalizeCountryCode(input) {
    ...
    return COUNTRY_TO_CODE[compact] || null;
}
...
/**
 * Build query based on OR within each filter
 * AND between all filter groups
 */
function buildFilterQuery({ helps, goals, categories, employeeCount, gender, country }) {
    const andClauses = [];

    // Helps — user must have ANY of the helps in request
    if (Array.isArray(helps) && helps.length > 0) {
        andClauses.push({ "userInfo.helps": { $in: helps } });
    }

    // Goals — user’s goal must match ANY in list
    if (Array.isArray(goals) && goals.length > 0) {
        const regexes = goals.map((g) => new RegExp(escapeRegExp(g), "i"));
        andClauses.push({ "userInfo.goals": { $in: regexes } });
    }

    // Categories
    if (Array.isArray(categories) && categories.length > 0) {
        const regexes = categories.map((c) => new RegExp(escapeRegExp(c), "i"));
        andClauses.push({ "userInfo.categories": { $in: regexes } });
    }

    // Employee Count
    if (Array.isArray(employeeCount) && employeeCount.length > 0) {
        const regexes = employeeCount.map((e) => new RegExp(escapeRegExp(e), "i"));
        andClauses.push({ "userInfo.employeeCount": { $in: regexes } });
    }

    // Gender
    if (Array.isArray(gender) && gender.length > 0) {
        const regexes = gender.map((g) => new RegExp("^" + escapeRegExp(g) + "$", "i"));
        andClauses.push({ gender: { $in: regexes } });
    }

    // Country (check countryCode in user.phonenumbers)
    if (Array.isArray(country) && country.length > 0) {
        const codes = country
            .map((c) => normalizeCountryCode(c))
            .filter((x) => x);
        if (codes.length > 0) {
            const regexes = codes.map((code) => new RegExp("^" + escapeRegExp(code) + "$", "i"));
            andClauses.push({
                phonenumbers: {
                    $elemMatch: {
                        countryCode: { $in: regexes },
                    },
                },
            });
        }
    ...
    // Base: must be subscribed and have valid email
    const baseAnd = [
        { userSubscribed: true },
        { email: { $exists: true, $ne: "" } },
    ];
    ...
    if (andClauses.length === 0) {
        return { $and: baseAnd }; // no filters → all subscribed
    }
...
exports.sendEmailToFilteredUsers = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            status: "success",
            message: `Email sent successfully to ${totalSent} users.`,
            sentCount: totalSent,
        });
    } catch (err) {
        console.error("Admin email send error:", err);
