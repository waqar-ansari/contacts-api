...
exports.sendSubscriptionExpiryAlerts = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `🔍 Searching for users expiring on ${targetStart.format("YYYY-MM-DD")}`
    );
    ...
    await Promise.all(
      allUsers.map((user) =>
        stripeLimiter(async () => {
          const { plan, expiresAt } = await getUserCurrentPlan(
            user,
            useTestMode
          );
          if (!expiresAt) return;
    ...
    console.log(
      `✅ Found ${matchedUsers.length} users whose plan expires in ${days} days.`
    );

    if (matchedUsers.length === 0)
      return res
        .status(200)
        .json({ message: `No users expiring in ${days} day(s).` });
