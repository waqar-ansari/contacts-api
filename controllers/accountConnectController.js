...
exports.googleCallback = async (req, res) => {
  ...
  try {
    ...
    if (type === "mobile") {
      const redirectUrl = `contactsmanagement://callback?status=success&googleId=${encodeURIComponent(
        user.googleId
      )}&googleEmail=${encodeURIComponent(
        user.googleEmail
      )}&message=${encodeURIComponent("Google Connected Successfully")}`;
...
exports.microsoftCallback = async (req, res) => {
  ...
  try {
    const tokenResponse = await axios.post(
      ...
      new URLSearchParams({
        client_id: process.env.MICROSOFT_CLIENT_ID,
        client_secret: process.env.MICROSOFT_CLIENT_SECRET, // ✅ REQUIRED
        code,
        redirect_uri: process.env.MICROSOFT_REDIRECT_URI,
    ...
    user.microsoftId = profileRes.data.id;
    user.microsoftEmail =
