  ...
  try {
    ...
    // ---------- MICROSOFT ----------
      ...
      if (!user.microsoftConnected || !user.microsoftAccessToken || !user.microsoftEmail) {
        return res.status(401).json({
          status: "error",
          message: "Microsoft account not connected on server. Please connect Microsoft from the web dashboard before sending email.",
        });
      }
      ...
      if (fromEmail && fromEmail.trim().toLowerCase() !== user.microsoftEmail.trim().toLowerCase()) {
        return res.status(400).json({
          status: "error",
          message: "Provided fromEmail does not match the connected Microsoft account. Use the connected Microsoft email or reconnect.",
        });
      }
      ...
      try {
        ...
        return res.status(200).json({ status: "success", message: "Email sent successfully via Microsoft Graph." });
      } catch (sendErr) {
    ...
    // ---------- SMTP ----------
      ...
      if (!user.smtpConnected) {
        return res.status(401).json({
          status: "error",
          message: "SMTP not connected for this user. Please connect SMTP from the web dashboard before sending email.",
        });
      }
      ...
      try {
        await transporter.sendMail(mailOptions);
        return res.status(200).json({ status: "success", message: "Email sent successfully via SMTP." });
      } catch (sendErr) {
